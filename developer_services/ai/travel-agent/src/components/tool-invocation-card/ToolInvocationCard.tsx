        ...
        >
          ...
          {!needsConfirmation && toolUIPart.state === "output-available" && (
            <div className="mt-3 border-t border-[#F48120]/10 pt-3">
              ...
              <div className="bg-background/80 p-2 rounded-md text-xs overflow-auto whitespace-pre-wrap break-words max-w-[450px]">
                {(() => {
                  const result = toolUIPart.output;
                  
                  // Check if it's a PerfectStay result
                  if (typeof result === 'object' && result !== null && 'products' in result) {
                    const perfectStayResult = result as { products?: any[], totalResults?: number, aiSummary?: string };
                    return (
                      <div className="text-sm">
                        {perfectStayResult.aiSummary && (
                          <p className="mb-2 text-blue-600 dark:text-blue-400 font-medium">
                            {perfectStayResult.aiSummary}
                          </p>
                        )}
                        <p className="text-green-600 dark:text-green-400">
                          ✓ {perfectStayResult.totalResults || 0} offre{perfectStayResult.totalResults !== 1 ? 's' : ''} trouvée{perfectStayResult.totalResults !== 1 ? 's' : ''}
                        </p>
                      </div>
                    );
                  }
